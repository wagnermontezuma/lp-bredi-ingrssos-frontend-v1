...
const EventTypes = () => {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      setXMax(maxX);
      wrapper.style.height = `${maxX + pin.clientHeight}px`;
    };
    ...
    return (
      <section id="event-types" className="bg-bredi-primary text-white">
        <div className="mx-auto max-w-6xl px-6 py-16 md:py-24">
          <h2 className="text-center text-3xl font-extrabold uppercase tracking-tight md:text-5xl">
            PARA TODOS OS TIPOS DE EVENTO
          </h2>
          <p className="mt-4 text-center text-base text-white/70 md:text-lg">
            Da balada ao festival, do corporativo ao casamento. Temos a solução ideal para você.
          </p>
          <div className="mt-12 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {CARDS.map((card) => (
              <div key={card.id} className="group relative h-80 overflow-hidden rounded-2xl ring-1 ring-white/10">
                <div
  ...
  return (
    <section id="event-types" aria-label="Para Todos os Tipos de Evento" className="relative bg-bredi-primary text-white">
      <div ref={wrapperRef} className="relative">
        <div ref={pinRef} className="sticky top-0 h-screen overflow-hidden [overscroll-behavior:contain]">
          <div className="sticky top-0 z-20 mx-auto max-w-6xl px-6 pb-6 pt-10 md:pt-16">
            <h2 className="text-center text-3xl font-extrabold uppercase tracking-tight md:text-5xl">
              PARA TODOS OS TIPOS DE EVENTO
            </h2>
            <p className="mt-3 text-center text-base text-white/70 md:text-lg">
              Da balada ao festival, do corporativo ao casamento. Temos a solução ideal para você.
            </p>
          </div>
          ...
          <motion.div
            ref={trackRef}
            style={{ x }}
            className="mx-auto flex h-[70vh] items-center px-[10vw] will-change-transform"
          >
            {CARDS.map((card) => (
              <article
                key={card.id}
                className="relative mx-6 h-[70vh] w-[68vw] shrink-0 overflow-hidden rounded-2xl shadow-2xl ring-1 ring-white/10 md:mx-10"
